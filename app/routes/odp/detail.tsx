...
export default function OdpDetail() {
  const { id } = useParams();

  const [odp, setOdp] = useState<any>(null);

  useEffect(() => {
    if (id) {
      getOdpId(id).then(setOdp);
    }
  }, [id]);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between rounded-2xl bg-white p-6 shadow">
        <div>
          <h1 className="text-3xl font-bold">{odp.name}</h1>
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <div className="rounded-2xl bg-white p-6 shadow">
          <h2 className="mb-5 text-xl font-semibold">Informasi ODP</h2>
          ...
          <div className="space-y-4">
            ...
            <div>
              <p className="text-sm text-slate-500">Rasio</p>

              <p className="font-semibold">{odp.rasio ?? "-"}</p>
            </div>
